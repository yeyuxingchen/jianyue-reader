...
const electronAPI = {
  ...
  fs: {
    ...
    // 文件目录（简记模式左侧"文件目录"面板）
    scanFileTree: (rootPath: string) => ipcRenderer.invoke('fs:scanFileTree', rootPath),
    createChapter: (parentDir: string, name: string) => ipcRenderer.invoke('fs:createChapter', parentDir, name),
    createDirectory: (parentDir: string, name: string) => ipcRenderer.invoke('fs:createDirectory', parentDir, name),
    renameNode: (oldPath: string, newName: string) => ipcRenderer.invoke('fs:renameNode', oldPath, newName),
    createEpubDirectory: (parentDir: string) => ipcRenderer.invoke('fs:createEpubDirectory', parentDir),
    pathExists: (p: string) => ipcRenderer.invoke('fs:pathExists', p),
    exportEpub: (dirPath: string) => ipcRenderer.invoke('epub:exportFromDir', dirPath),
    setEpubCover: (dirPath: string) => ipcRenderer.invoke('epub:setCover', dirPath),
    getEpubCover: (dirPath: string) => ipcRenderer.invoke('epub:getCover', dirPath),
    saveChapterImage: (dirPath: string, base64DataUrl: string) => ipcRenderer.invoke('epub:saveChapterImage', dirPath, base64DataUrl),
    saveChapterImageFromFile: (dirPath: string, sourcePath: string) => ipcRenderer.invoke('epub:saveChapterImageFromFile', dirPath, sourcePath),
    registerChapterDir: (dirPath: string) => ipcRenderer.invoke('epub:registerChapterDir', dirPath),
  },
  ...
  dialog: {
    ...
    showNoteSaveDialog: (data: string, defaultDir?: string) => ipcRenderer.invoke('dialog:showNoteSaveDialog', data, defaultDir),
  },
